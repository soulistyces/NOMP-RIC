...
/* The daemon interface interacts with the coin daemon by using the rpc interface in order to make it work it needs, as constructor, an array of objects containing
...
// Private members
	var _this = this;
	logger = logger || function(severity, message) {console.log(severity + ': ' + message);};
	...
	function isOnline(callback) {
		cmd('getpeerinfo', [], function(results) {
			var allOnline = results.every(function(result) {return !results.error;});
			callback(allOnline);
			if (!allOnline)
	...
	function performHttpRequest(instance, jsonData, callback) {
		...
		var req = http.request(options, function(res) {
			...
			res.on('end', function(){parseJson(res, data);});
		});
	...
	function batchCmd(cmdArray, callback) {
		...
		performHttpRequest(instances[0], serializedRequest, function(error, result) {callback(error, result);});
	}
	/* Sends a JSON RPC (http://json-rpc.org/wiki/specification) command to every configured daemon.
	   The callback function is fired once with the result from each daemon unless streamResults is set to true. */
	function cmd(method, params, callback, streamResults, returnRawData) {
		...
		async.each(instances, function(instance, eachCallback) {
			...
			performHttpRequest(instance, requestJson, function(error, result, data) {itemFinished(error, result, data);});
		}, function() {
			if (!streamResults)
				callback(results);
		});
