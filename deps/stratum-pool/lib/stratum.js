...
/* Defining each client that connects to the stratum server. Emits:
...
// Private members
	...
	function handleAuthorize(message, replyToSocket) {
		...
		options.authorizeFn(_this.remoteAddress, options.socket.localPort, _this.workerName, _this.workerPass, function(result) {
			...
			if (result.disconnect === true) // If the authorizer wants us to close the socket lets do it.
...
// Public members
	this.sendDifficulty = function(difficulty) { // If the given difficulty is valid and new it'll send it to the client.
	...
	this.manuallyAuthClient = function(username, password) {handleAuthorize({id: 1, params: [username, password]}, false /*do not reply to miner*/);};
...
/* The actual stratum server. It emits the following Events:
...
// Private members
	...
	this.broadcastMiningJobs = function(jobParams) {
		...
		rebroadcastTimeout = setTimeout(function() {_this.emit('broadcastTimeout');}, options.jobRebroadcastTimeout*1000);
	};
	...
	(function init() {
		...
		Object.keys(options.ports).forEach(
			function(port) {net.createServer({allowHalfOpen: false},
			function(socket) {_this.handleNewClient(socket);}
