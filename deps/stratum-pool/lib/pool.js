...
var pool = module.exports = function pool(options, authorizeFn) {
	...
	if (!(options.coin.algorithm in algos)) {
		emitErrorLog('The ' + options.coin.algorithm + ' PoW is not supported.');
		throw new Error();
	}
	...
	function GetFirstJob(finishedCallback) {
		GetBlockTemplate(function(error, result) {
			...
			if (portWarnings.length > 0 && (!process.env.forkId || process.env.forkId === '0')) { // Only let the first fork show synced status or the log wil look flooded with it
	...
	function OutputPoolInfo() {
		...
		var infoLines = [startMessage,
				...
				'Block Height:\t' + _this.jobManager.currentJob.rpcData.height,
				'Connected Peers:\t' + options.initStats.connections,
				...
				'Network Mining Power:\t' + options.initStats.networkHashRate,
		...
		var checkSynced = function(displayNotSynced) {
			_this.daemon.cmd('getblocktemplate', [{"capabilities": ["coinbasetxn", "workid", "coinbase/append"], "rules": ["segwit"]}], function(results) {
				...
				if (synced)
					...
					if (!process.env.forkId || process.env.forkId === '0') // Only let the first fork show synced status or the log wil look flooded with it
						generateProgress();
				}
			});
		};
		...
		var generateProgress = function() {
			...
			_this.daemon.cmd(cmd, [], function(results) {
				...
				_this.daemon.cmd('getpeerinfo', [], function(results) {
					...
					var totalBlocks = peers.sort(function(a, b) {return b.startingheight - a.startingheight;})[0].startingheight;
	...
	function SetupPeer() {
		...
		_this.peer.on('connected', function() {emitLog('p2p connection successful');}
		).on('connectionRejected', function() {emitErrorLog('p2p connection failed - likely incorrect p2p magic value');}
		).on('disconnected', function() {emitWarningLog('p2p peer node disconnected - attempting reconnection...');}
		).on('connectionFailed', function(e) {emitErrorLog('p2p connection failed - likely incorrect host or port');}
		).on('socketError', function(e) {emitErrorLog('p2p had a socket error ' + JSON.stringify(e));}
		).on('error', function(msg) {emitWarningLog('p2p had an error ' + msg);}
		).on('blockFound', function(hash) {_this.processBlockNotify(hash, 'p2p');});
	}
	...
	function SetupJobManager() {
		...
		_this.jobManager.on('newBlock', function(blockTemplate) {
			if (_this.stratumServer) // Check if stratumServer has been initialized yet
			...
			// If we calculated that the block solution was found, before we emit the share, lets submit the block, then check if it was accepted using RPC getblock
			if (!isValidBlock)
				emitShare();
			else{
		...
		}).on('log', function(severity, message){_this.emit('log', severity, message);});
	}
	...
	function SetupDaemonInterface(finishedCallback) {
		...
		_this.daemon.once('online', function() {finishedCallback();}
		).on('connectionFailed', function(error) {emitErrorLog('Failed to connect daemon(s): ' + JSON.stringify(error));}
		).on('error', function(message) {emitErrorLog(message);});
		_this.daemon.init();
	}
	...
	function DetectCoinData(finishedCallback) {
		...
		_this.daemon.batchCmd(batchRpcCalls, function(error, results) {
			...
			options.testnet = (rpcResults.getblockchaininfo.chain != 'main');
			options.protocolVersion = rpcResults.getnetworkinfo.protocolversion;
			var difficulty = rpcResults.getblockchaininfo.difficulty;
			options.initStats = {
	...
	function StartStratumServer(finishedCallback) {
		...
		_this.stratumServer.on('started', function() {
			...
			emitLog('No new blocks for ' + options.jobRebroadcastTimeout + ' s, updating transactions and rebroadcasting jobs');
			GetBlockTemplate(function(error, rpcData, processedBlock) {
				if (error || processedBlock) return;
				_this.jobManager.updateCurrentJob(rpcData);
			});
		}).on('client.connected', function(client) {
			client.on('difficultyChanged', function(diff) {_this.emit('difficultyUpdate', client.workerName, diff);}
				...
				var result =_this.jobManager.processShare(params.jobId, client.previousDifficulty, client.difficulty, client.extraNonce1, params.extraNonce2, params.nTime, params.nonce, client.remoteAddress, client.socket.localPort, params.name);
				resultCallback(result.error, result.result ? true : null);
			}).on('malformedMessage', function (message) {emitWarningLog('Malformed message from ' + client.getLabel() + ': ' + message);}
			).on('socketError', function(err) {emitWarningLog('Socket error from ' + client.getLabel() + ': ' + JSON.stringify(err));}
			).on('socketTimeout', function(reason) {emitWarningLog('Connected timed out for ' + client.getLabel() + ': ' + reason)}
			).on('socketDisconnect', function() {/*emitLog('Socket disconnected from ' + client.getLabel());*/}
			).on('kickedBannedIP', function(remainingBanTime) {emitLog('Rejected incoming connection from ' + client.remoteAddress + ' banned for ' + remainingBanTime + ' more seconds');}
			).on('forgaveBannedIP', function() {emitLog('Forgave banned IP ' + client.remoteAddress);}
			).on('unknownStratumMethod', function(fullMessage) {emitLog('Unknown stratum method from ' + client.getLabel() + ': ' + fullMessage.method);}
			).on('socketFlooded', function() {emitWarningLog('Detected socket flooding from ' + client.getLabel());}
			).on('tcpProxyError', function(data) {emitErrorLog('Client IP detection failed, tcpProxyProtocol is enabled yet did not receive proxy protocol message, instead got data: ' + data);}
			).on('bootedBannedWorker', function() {emitWarningLog('Booted worker ' + client.getLabel() + ' who was connected from an IP address that was just banned');}
	...
	// This method is being called from the blockNotify so that when a new block is discovered by the daemon. We can inform our miners about the newly found block
	...
	this.relinquishMiners = function(filterFn, resultCback) {
		...
		var stratumClients = [];
		Object.keys(origStratumClients).forEach(function (subId) {stratumClients.push({subId: subId, client: origStratumClients[subId]});});
		async.filter(
			stratumClients,
			filterFn,
				...
				process.nextTick(function() {
					resultCback(
						clientsToRelinquish.map(
							function (item) {return item.client;}
						)
					);
				});
			}
