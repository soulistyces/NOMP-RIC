...
try { // Try to give process ability to handle 100k concurrent connections
	...
	try {
		...
		if (uid) { // Set our server's uid to that user
			process.setuid(uid);
...
var startCliListener = function() {
	...
	listener.on('log', function(text) {logger.debug('Master', 'CLI', text);}).on('command', function(command, params, options, reply) {
		switch(command) {
			case 'blocknotify':
				Object.keys(cluster.workers).forEach(function(id) {
...
var startPaymentProcessor = function() {
	...
	worker.on('exit', function(code, signal) {
		...
		setTimeout(function() {startPaymentProcessor(poolConfigs);}, 2000);
	});
};
...
var startWebsite = function() {
	...
	worker.on('exit', function(code, signal) {
		...
		setTimeout(function() {startWebsite(portalConfig, poolConfigs);}, 2000);
	});
};
