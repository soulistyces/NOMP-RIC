...
module.exports = function(logger) {
	...
	async.filter(enabledPools, function(coin, callback) {
		SetupForPool(logger, poolConfigs[coin], function(setupResults) {callback(setupResults);});
	}, function(coins) {
...
function SetupForPool(logger, poolOptions, setupFinished) {
	...
	var daemon = new Stratum.daemon.interface([processingConfig.daemon], function(severity, message) {logger[severity](logSystem, logComponent, message);});
				...
				if (result.error) {
					...
					callback(true);
						...
						if (result.error) {
							...
							logger.error(logSystem, logComponent, 'Daemon does not own pool address - payment processing can not be done with this daemon, ' + JSON.stringify(result.response));
							callback(true);
						}
						else
							callback()
	...
	var satoshisToCoins = function(satoshis) {return parseFloat((satoshis / magnitude).toFixed(coinPrecision));};
	var coinsToSatoshies = function(coins) {return coins*magnitude;};
	...
	var processPayments = function() {
		...
		async.waterfall([
			// Call redis to get an array of rounds - which are coinbase transactions and block heights from submitted blocks.
			function(callback) {
				startRedisTimer();
				redisClient.multi([
			...
			function(workers, rounds, callback) {
				var batchRPCcommand = rounds.map(function(r) {return ['gettransaction', [r.txHash]];});
				...
				daemon.batchCmd(batchRPCcommand, function(error, txDetails) {
					...
					txDetails.forEach(function(tx, i) {
						...
						var generationTx = tx.result.details.filter(function(tx) {return tx.address === poolOptions.address;})[0];
			...
			// Does a batch redis call to get shares contributed to each round. Then calculates the reward amount owned to each miner for each round.
			function(workers, rounds, addressAccount, callback) {
				var shareLookups = rounds.map(function(r) {return ['hgetall', coin + ':shares:round' + r.height]});
				startRedisTimer();
					...
					rounds.forEach(function(round, i) {
						...
						switch (round.category) {
							...
							case 'orphan':
								...
								break;
							case 'generate':
								// We found a confirmed block! Now get the reward for it and calculate how much we owe each miner based on the shares they submitted during that block round.
								var reward = parseInt(round.reward*magnitude);
								var totalShares = Object.keys(workerShares).reduce(function(p, c) {return p + parseFloat(workerShares[c])}, 0);
				...
				var trySend = function (withholdPercent) {
					...
					daemon.cmd('sendmany', [addressAccount || '', addressAmounts], function (result) {
						...
						if (result.error && result.error.code === -6) {
							...
							logger.debug(logSystem, logComponent, 'Sent out a total of ' + (totalSent/magnitude) + ' to ' + Object.keys(addressAmounts).length + ' workers');
							if (withholdPercent > 0)
								logger.warning(logSystem, logComponent, 'Had to withhold ' + (withholdPercent*100) + '% of reward from miners to cover transaction fees. ' + 'Fund pool wallet with coins to prevent this from happening');
							callback(null, workers, rounds);
						}
					}, true, true);
				};
			...
			function(workers, rounds, callback) {
				...
				for (var w in workers) {
					...
					if (worker.balanceChange !== 0)
						balanceUpdateCommands.push(['hincrbyfloat', coin + ':balances', w, satoshisToCoins(worker.balanceChange)]);
					if (worker.sent !== 0) {
				...
				redisClient.multi(finalRedisCommands).exec(function(error, results){
					...
					if (error){
						...
						logger.error(logSystem, logComponent, 'Payments sent but could not update redis. ' + JSON.stringify(error) + ' Disabling payment processing to prevent possible double-payouts. The redis commands in ' + coin + '_finalRedisCommands.txt must be ran manually');
